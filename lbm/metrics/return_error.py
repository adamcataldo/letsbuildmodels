...
class ReturnError(Metric):
    ...
    @torch.inference_mode()
    def compute(self):
        predicted_returns = ((self.predicted_outputs - self.final_inputs) 
                             / self.final_inputs)
        actual_returns = ((self.actual_outputs - self.final_inputs) 
                          / self.final_inputs)
        avg_return = actual_returns.mean()
        avg_return_error = torch.abs(predicted_returns - actual_returns).mean()
        return avg_return_error.item() / avg_return.item()
