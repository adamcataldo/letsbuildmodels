...
class TestTimeSeriesDataset(unittest.TestCase):
    ...
    def test_simple_case(self):
        dataset = TimeSeriesDataset(self.df, 3, 1, False)
    ...
    def test_horizon(self):
        dataset = TimeSeriesDataset(self.df, 3, 2, False)
    ...
    def test_alignment(self):
        dataset = TimeSeriesDataset(self.df, 3, 1, True)
        self.assertEqual(len(dataset), 7)
        x, y = dataset[2]
        expected_x = torch.tensor([[3, 8], [4, 7], [5, 6]], dtype=torch.float32)
    ...
    def test_horizon_and_alignment(self):
        dataset = TimeSeriesDataset(self.df, 3, 2, True)
        self.assertEqual(len(dataset), 6)
        x, y = dataset[2]
        expected_x = torch.tensor([[3, 8], [4, 7], [5, 6]], dtype=torch.float32)
...
class TestTimeSeriesPreprocessor(unittest.TestCase):
    ...
    def test_get_loaders(self):
        preprocessor = TimeSeriesPreprocessor(self.df, 3, 1, False)
